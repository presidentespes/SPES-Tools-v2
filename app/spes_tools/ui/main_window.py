...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        modules: list[tuple[str, str, str, Callable[[], None]]] = [
            (
                ...
                "Importa Nexi, BCC e Volksbank; controlla e crea il CSV TeamSystem.",
                self.open_banking,
            ),
            (
                "💳",
                "Bonifici SEPA",
                "Gestisci bonifici, beneficiari, causali e dati IBAN.",
                self.open_banking,
            ),
            (
                "💶",
                "Convertitore compensi",
                "Calcolo lordo ↔ netto con anteprima immediata ed esportazione PDF.",
                self.open_caf,
            ),
            (
                "📚",
                "Causali ABI",
                "Configura e personalizza i codici per Nexi, BCC e Volksbank.",
                self.open_abi,
            ),
            (
                "📂",
                "Storico operazioni",
                "Consulta conversioni, esportazioni e calcoli effettuati.",
                self.open_history,
            ),
            (
                "ℹ",
                "Informazioni",
                "Versione, identità dell'applicazione e moduli disponibili.",
                self.open_info,
            ),
        ]
        ...
        for index, (icon, title, description, callback) in enumerate(modules):
            grid.addWidget(
                self._card(icon, title, description, callback),
                index // 2,
                index % 2,
            )
        ...
        status.setStyleSheet(
            "QStatusBar {background: #ffffff; color: #53657a; border-top: 1px solid #d6dde5;}"
        )
    ...
    def _build_header(self) -> QWidget:
        ...
        header.setStyleSheet(
            "QWidget {background: #073b84; border-radius: 15px;}"
        )
        ...
        if not pixmap.isNull():
            logo.setPixmap(
                pixmap.scaled(116, 116, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            )
        ...
        title.setStyleSheet(
            "font-size: 31px; font-weight: 750; color: white;"
        )
    ...
    def _card(
        ...
        card.setStyleSheet(
            "QWidget {background: white; border: 1px solid #d6dde5; border-radius: 13px;}"
        )
        ...
        button.setStyleSheet(
            ...
            "border: none; border-radius: 7px; font-weight: 650;}"
