...
class CafWindow(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        layout.addWidget(QLabel("Calcolo indicativo configurabile. Non sostituisce un cedolino o un calcolo fiscale professionale."))

        form = QFormLayout()
        self.mode = QComboBox()
        self.mode.addItems(["Da lordo a netto", "Da netto a lordo"])
        form.addRow("Tipo calcolo", self.mode)

        self.profile = QComboBox()
        self.profile.addItems(["Dipendente", "Pensionato", "Collaboratore sportivo"])
        ...
        form.addRow("Aliquota imposte", self.tax_rate)
        ...
        form.addRow("Contributi", self.contrib_rate)
        ...
        form.addRow("Detrazioni/abbattimenti", self.deductions)

        self.months = QSpinBox()
        self.months.setRange(1, 14)
        self.months.setValue(12)
        form.addRow("Mensilita", self.months)
        ...
        self.result.setStyleSheet("font-size: 16px; padding: 12px; background: white; border: 1px solid #d6dde5;")
    ...
    def _profile_changed(self, profile: str) -> None:
        defaults = {
            ...
            "Pensionato": (23.0, 0.0),
            "Collaboratore sportivo": (23.0, 24.0),
        }
    ...
    def calculate(self) -> None:
        ...
        if self.mode.currentIndex() == 0:
            ...
            net = amount
            if total_rate >= 0.999:
                QMessageBox.warning(self, "Aliquote non valide", "La somma delle aliquote deve essere inferiore al 100%.")
                return
    ...
    def export_pdf(self) -> None:
        ...
        if self.last_result is None:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Salva calcolo PDF", "calcolo_caf.pdf", "PDF (*.pdf)")
        if not path:
            return
        ...
        try:
            ...
            add_history(module="CAF Tools", source="Calcolo manuale", output=path, rows=1, details=str(self.last_result.get("profilo", "")))
